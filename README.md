...
| 10 | **GROUP CONTACT/TIE** | (a) group rỗng/stale nằm trong pair → ERROR; (b) so 2 phía mỗi pair: số mặt + loại (tri/quad) + **DIỆN TÍCH** — số mặt lệch nhưng diện tích khớp (vd 320 tri vs 160 quad) = hợp lý và ghi rõ logic; khi diện tích lệch thật thì đo thêm **độ phủ tương hỗ**: 2 phía vẫn phủ nhau đủ (≥80%) = tiling/loại element khác nhau → hợp lệ (vd master dạng tấm SURFACE_ELEMENT có khe), còn độ phủ hụt → WARN thiếu element thật; (c) surface bị tách **mảng rời rạc**; (d) **GROUP CHỒNG NHAU** — 2 group dùng chung mặt trong các pair khác nhau → WARN double-count; (e) khuyến nghị lưới mịn làm slave; group assign mà không dùng → WARN (nhóm 2) | ERROR/WARN/INFO |
